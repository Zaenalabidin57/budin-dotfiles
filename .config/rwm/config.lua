...
function tagkeys (keys)
   for i, xk in ipairs({XK_z, XK_x, XK_c, XK_v, XK_b, XK_6, XK_7, XK_8, XK_9}) do
	  arg = {Ui = 1 << (i - 1)}
...
keys = {
   ...
   key(modkey, XK_n, spawn, {V = filecmd}),
   key(modkey, XK_o, spawn, {V = browser}),
   key(modkey, XK_Return, spawn, {V = termcmd}),
   key(modkey, XK_s, togglescratch, {V = scratchpadcmd}),
   key(modkey, XK_p, togglebar, {I = 0}),
   key(modkey, XK_j, focusstack, {I = 1}),
   key(modkey, XK_k, focusstack, {I = -1}),
   key(s_mod, XK_j, pushstack, {I = 1}),
   key(s_mod, XK_k, pushstack, {I = -1}),
   key(s_mod, XK_i, incnmaster, {I = 1}),
   key(s_mod, XK_u, incnmaster, {I = -1}),
   key(modkey, XK_h, setmfact, {F = -0.05}),
   key(modkey, XK_l, setmfact, {F = 0.05}),
   key(s_mod, XK_Return, zoom, {I = 0}),
   key(modkey, XK_Tab, view, {Ui = 0}),
   key(modkey, XK_q, killclient, {I = 0}),
   key(modkey, XK_t, setlayout, {L = 0}),
   key(s_mod, XK_f, setlayout, {L = 1}),
   ...
   key(modkey, XK_f, fullscreen, {I = 0}),
}
tagkeys(keys)
